...
def init_net(net, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    global gen_device
    gen_device = torch.device('cuda:{}'.format(gpu_ids[0])) if gpu_ids else torch.device(
        'cpu')
    return net


def define_UnetMask(input_nc, nuf):
    netG = UnetMask(input_nc, output_nc=4, nuf=nuf)
    return netG


def define_Refine(input_nc, output_nc, nrf, norm):
    norm = get_norm_layer(norm)
    netG = Refine(input_nc, output_nc, nrf, norm)
    return netG


def define_STN(target_control_points):
    netG = STNNet(target_control_points)
    return netG


def define_TPS(hight: int = 256, weidth: int = 192, target_control_points=None):
    netG = TPSGridGen(hight, weidth, target_control_points)
    return netG
...
class GANLoss(nn.Module):
    ...
    def __init__(self, gan_mode, target_real_label=1.0, target_fake_label=0.0):
        ...
        self.register_buffer('real_label', torch.tensor(target_real_label).to(gen_device))
        self.register_buffer('fake_label', torch.tensor(target_fake_label).to(gen_device))
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
        ...
        model.append(nn.Conv2d(input_nc, ngf, kernel_size=7, padding=0, bias=use_bias))
        ...
        model += [nn.Conv2d(ngf * 2, ngf // 2, kernel_size=3, stride=2, padding=1, bias=use_bias),
                  norm_layer(ngf // 2),
                  nn.ReLU()]
        model.append(ResnetBlock(ngf // 2, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout,
                                 use_bias=use_bias))
